...
cmd({
  pattern: "sinhalasub",
  react: "📽️",
  desc: "Enjoy cinema from CHAMI's treasury of films with Sinhala subtitles",
  ...
  try {
    ...
    if (!searchData) {
      ...
      if (!searchData.status || !searchData.result.data || searchData.result.data.length === 0) {
        throw new Error("No films found in chami");
      }
    ...
    filmList += `${frozenTheme.resultEmojis[9]} 𝗖𝗛𝗔𝗠𝗜 𝗠𝗗 𝗕𝗬 𝗖𝗛𝗔𝗠𝗜 𝗘𝗗𝗜𝗧𝗭`;
      ...
      if (replyText.toLowerCase() === "done") {
        ...
        await conn.sendMessage(from, {
          text: frozenTheme.box(" chami", 
            " Cinematic quest ended!\nReturn to the Ice chamianytime"),
          ...frozenTheme.getForwardProps()
        }, { quoted: message });
        return;
      }
